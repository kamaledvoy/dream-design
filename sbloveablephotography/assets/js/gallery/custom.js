jQuery(function ($) {
  ...
  // Loading
  $("body.loading_page").prepend(
    '<div class="loadingtt"><img src="images/loading.gif" /></div>'
  );
  ...
  jQuery("ul.tt-toggle")
    .find(".toggle-content.active")
    .siblings(".toggle-head")
    .trigger("click");
  // 4Mob
  $(".headdown nav").before(
    '<div id="mobilepro"><i class="icon-reorder icon-remove"></i></div>'
  );
  $(".headdown .sf-menu li").addClass("xpopdrop");
  $("#mobilepro").click(function () {
    $(".headdown .sf-menu")
      .slideToggle("slow", "easeInOutExpo")
      .toggleClass("xactive");
    $("#mobilepro i").toggleClass("icon-reorder");
  });
  $("body").click(function () {
    $(".headdown .xactive")
      .slideUp("slow", "easeInOutExpo")
      .removeClass("xactive");
  ...
  if ($(".filter")[0]) {
    ...
    $(".filter a").on("click", function (e) {
      ...
      if ($filterClass === "all") {
        ...
        var $filteredPortfolio = $portfolioClone.find(
      ...
      $(".portfolio").quicksand(
        ...
        function () {
          ...
          if (
            !(jQuery.browser.msie && parseInt(jQuery.browser.version, 10) < 6)
          ) {
            jQuery(".hover-fx").each(function () {
              var overImg = jQuery(this).find(".overlay");
              jQuery(this).hover(
  ...
  if ($("#flickr-photos")[0]) {
    $("#flickr-photos").jflickrfeed({
      ...
      itemTemplate:
        "<li>" +
        '<a href="{{image_b}}" data-gal="photo[flickr]"><img src="{{image_s}}" alt="{{title}}" /></a>' +
        "</li>",
      ...
      $("#contactForm .requiredField").each(function () {
        if (jQuery.trim($(this).val()) === "") {
          ...
          if (!emailReg.test(jQuery.trim($(this).val()))) {
            ...
            $(this).addClass("fielderror");
            $("#contactForm span").html(
              "<strong>Is incorrect your email address</strong>"
            );
            hasError = true;
          }
        }
      ...
      if (!hasError) {
        $("#contactForm").slideDown("normal", function () {
          ...
          $("#contactForm #sendMessage")
            ...
            .val("Sending message. Please wait...");
        });
        var formInput = $(this).serialize();
        $.post($(this).attr("action"), formInput, function (data) {
          $("#contactForm").slideUp("normal", function () {
            $(this).before(
              '<div class="notification-box notification-box-success"><p><i class="icon-ok"></i>Thanks!</strong> Your email was successfully sent. We check Our email all the time, so we should be in touch soon.</p></div>'
            );
          });
        });
      }
      ...
      .on("scroll", function () {
        ...
        if (window_width > 768) {
          if (wind_scr < 200) {
            ...
            if (
              $("#header").data("sticky") === false ||
              typeof $("#header").data("sticky") === "undefined"
            ) {
  ...
  // Example Load News
  $(".load-news").append(
    '<img style="display: none;margin: 0 auto" src="images/loading2.gif"><h4 style="display: none;color:#ccc;border: 0">Sorry! Not More News.</h4>'
  );
  $(".load-news a")
    .removeAttr("href")
    .click(function () {
